...
def is_incomplete_name(name):
    ...
    # Check if it's very short (1-2 letters)
    if len(name) <= 2:
        return True
    ...
    location_patterns = [
        r'^(Akron|Cleveland|Columbus|Public|Private)$',
    ]
    ...
    # Check if it looks like abbreviated/incomplete (e.g., "aV")
...
def extract_name_from_email(email):
    ...
    if match:
        # Extract the capitalized part as last name
...
if __name__ == '__main__':
    ...
    for i, entry in enumerate(incomplete, 1):
        print(f"{i}. Line {entry['line_num']}:")
        print(f"   Company: {entry['company']}")
        print(f"   Current: {entry['first_name']} {entry['last_name']}")
        print(f"   Email: {entry['email']}")

        if entry['inferred_first'] or entry['inferred_last']:
            print(f"   → Can infer: {entry['inferred_first'] or '?'} {entry['inferred_last'] or '?'}")
        else:
            print(f"   → Cannot infer from email")
